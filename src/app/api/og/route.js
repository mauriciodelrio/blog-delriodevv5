...
export async function GET(request) {
  try {
    ...
    const displayTitle =
      type === 'home' ? (locale === 'es' ? 'Mauricio Del Río' : 'Mauricio Del Río') : title || 'Blog Post';

    const displaySubtitle =
      type === 'home' ? (locale === 'es' ? 'Desarrollador Senior Full Stack' : 'Senior Full Stack Developer') : '';
    ...
    return new ImageResponse(
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', width: '65%' }}>
          ...
          {type === 'home' && (
            <div
              style={{ fontSize: '22px', color: '#fbbf24', fontWeight: '600', marginBottom: '20px', display: 'flex' }}
            >
              🚀 {displayExperience}
            </div>
          )}
          <div style={{ fontSize: '18px', color: 'rgba(255, 255, 255, 0.8)' }}>delrio.dev</div>
        </div>
        {type === 'home' && (
