  ...
  return (
    <>
      {(showCategories || showTags) && (
        <div className="fixed top-0 left-0 w-full h-full bg-transparent z-10" onClick={closeAllBoxes} />
      )}

      <div
        className="flex flex-col w-full justify-center items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-16 relative"
      ...
      >
        <div className="flex flex-col tablet:flex-row tablet:justify-between tablet:items-center w-full mb-8 bg-gray-50 rounded-lg p-6 gap-4">
          <div className="flex flex-col mobile:flex-row mobile:flex-wrap items-start mobile:items-center gap-4">
            <h2 className="text-xl font-bold mobile:text-lg">{dictionary.posts.filters}</h2>
          ...
          <div className="flex flex-col mobile:flex-row mobile:items-center gap-4">
            <h3 className="text-lg font-semibold mobile:text-base">{dictionary.posts.orderBy}</h3>
            <button
              className="flex items-center gap-2 text-sm px-4 py-2 hover:bg-white hover:shadow-md rounded-lg transition-all duration-200 bg-white/50"
              onClick={handleOrder}
            >
              {!changedOrder ? dictionary.posts.newestFirst : dictionary.posts.oldestFirst}
            </button>
          </div>
        </div>

        <div className="grid gap-8 w-full mobile:grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-3 justify-center">
          <Card posts={contentToRender} locale={locale} getBadge={getBadge} badgesLoading={badgesLoading} />
        </div>
      </div>
    </>
  );
